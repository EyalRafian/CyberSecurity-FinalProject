...
function password_policy($password)
{
    ...
    */
    ...
    if (strlen($password) < $password_policy['length']) {
    ...
    if ($password_policy['specialChars']) {
...
function create_new_client($firstName, $lastName, $id, $phoneNumber, $address, $dealPackage, $sector)
{
    /*
    ...
    HOW TO FIX USING PARAMETERIZE QUERY:
        ...
        global $db;
        clients(firstName, lastName, id, phoneNumber, address, dealPackage, sector)";
        $query.= "VALUES(:firstNAme,:lastNAme,:id,:phoneNumber,:address,:dealPackage,:sector)";
        $stmt = $db->prepare($query)
        ...
        $stmt->bindValue('dealPackage',$dealPackage);
    ...
    */
    global $db;
    $query = "INSERT INTO clients(firstName, lastName, idNum, phoneNumber, address, dealPackage, sector)";
    $query .= "VALUES('$firstName','$lastName','$id','$phoneNumber','$address','$dealPackage','$sector')";
    $db->query($query);
    return $db->lastInsertId();
}
...
function hash_password($password)
{
    ...
    */
    $key = $password;
    return hash_hmac('sha1', $password . "Secret Salt!", $key);
}
...
function send_email($user, $token)
{
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'hitproject32@gmail.com';                     // SMTP username
        $mail->Password   = 'hit1234512345';                     // SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = 587;

        //Recipients
        $mail->setFrom('hitproject32@gmail.com', 'Mailer');
        $mail->addAddress($user['email'], $user['username']);       // Add a recipient

        // Content
        $mail->isHTML(true);                                        // Set email format to HTML
        $mail->Subject = 'Rest Token';
        $mail->Body    = '<h1>This is the token for reset your password sent from ilan, ido, lior and eyal.. <b>' . $token . '</b></h1>';
        $mail->send();
        return true;
    } catch (Exception $e) {
        die("error in mail method: " . $e->getMessage());
    }
}
